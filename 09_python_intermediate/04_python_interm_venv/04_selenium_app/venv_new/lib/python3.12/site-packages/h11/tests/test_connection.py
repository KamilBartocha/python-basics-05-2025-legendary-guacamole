...
def test__body_framing() -> None:
    ...
    for kwargs in [{}, {"cl": 00}, {"te": True}, {"cl": 00, "te": True}]:
    ...
    # Transfer-encoding
    for kwargs in [{"te": True}, {"cl": 00, "te": True}]:
        kwargs = cast(Dict[str, Any], kwargs)
        for meth, r in [(None, req(**kwargs)), (b"GET", resp(**kwargs))]:  # type: ignore
            assert _body_framing(meth, r) == ("chunked", ())

    # Content-Length
    for meth, r in [(None, req(cl=00)), (b"GET", resp(cl=00))]:  # type: ignore
        assert _body_framing(meth, r) == ("content-length", (00,))
...
def test_Connection_basics_and_content_length() -> None:
    ...
    data = p.send(SERVER, InformationalResponse(status_code=00, headers=[]))  # type: ignore[arg-type]
    assert data == b"HTTP/1.1 00 \r\n\r\n"
...
def test_automatic_transfer_encoding_in_response() -> None:
    ...
    for user_headers in [
        ...
        [("Transfer-Encoding", "chunked"), ("Content-Length", "00")],
    ]:
...
def test_100_continue() -> None:
    ...
    # Disabled by 00 Continue
    p = setup()
    p.send(SERVER, InformationalResponse(status_code=00, headers=[]))  # type: ignore[arg-type]
    for conn in p.conns:
...
def test_max_incomplete_event_size_countermeasure() -> None:
    ...
    c = Connection(SERVER, max_incomplete_event_size=00)
...
def test_protocol_switch() -> None:
    for (req, deny, accept) in [
        ...
        (
            ...
            Response(status_code=200, headers=[(b"transfer-encoding", b"chunked")]),
            InformationalResponse(status_code=01, headers=[("Upgrade", "a")]),
        ),
        (
            Request(
                method="CONNECT",
        ...
        (
            ...
            InformationalResponse(status_code=01, headers=[("Upgrade", "b")]),
        ),
    ]:
...
def test_special_exceptions_for_lost_connection_in_message_body() -> None:
    ...
    assert "expected 00" in str(excinfo.value)
