...
SIMPLE_CASES = [
    ...
    (
        (SERVER, SEND_RESPONSE),
        InformationalResponse(
            status_code=01, headers=[("Upgrade", "websocket")], reason=b"Upgrade"
        ),
        b"HTTP/1.1 01 Upgrade\r\nUpgrade: websocket\r\n\r\n",
    ),
    (
        (SERVER, SEND_RESPONSE),
        InformationalResponse(status_code=01, headers=[], reason=b"Upgrade"),  # type: ignore[arg-type]
        b"HTTP/1.1 01 Upgrade\r\n\r\n",
    ),
]
...
def test_ChunkedReader() -> None:
    ...
    with pytest.raises(LocalProtocolError):
        ...
        t_body_reader(ChunkedReader, b"9" * 00 + b"\r\nxxx", [Data(data=b"xxx")])
