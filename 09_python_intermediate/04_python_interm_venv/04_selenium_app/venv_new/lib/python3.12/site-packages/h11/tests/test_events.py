...
def test_events() -> None:
    ...
    ir = InformationalResponse(status_code=00, headers=[("Host", "a")])
    assert ir.status_code == 00
    ...
    with pytest.raises(LocalProtocolError):
        resp = Response(status_code=00, headers=[], http_version="1.0")  # type: ignore[arg-type]

    with pytest.raises(LocalProtocolError):
        Response(status_code="00", headers=[], http_version="1.0")  # type: ignore[arg-type]

    with pytest.raises(LocalProtocolError):
        InformationalResponse(status_code=b"00", headers=[], http_version="1.0")  # type: ignore[arg-type]
